...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'new-moves':
				dialogue = CoolUtil.coolTextFile(Paths.txt('new-moves/text'));
		}
		...
		switch (SONG.song.toLowerCase())
		{
			case 'new-moves':
				defaultCamZoom = 0.5;
				curStage = 'stage';
				var bg:FlxSprite = new FlxSprite(-1200, -500).loadGraphic(Paths.image('banjosky'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);

				var bg:FlxSprite = new FlxSprite(-1000, 500).loadGraphic(Paths.image('grass'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);

				var bg:FlxSprite = new FlxSprite(-1000, -500).loadGraphic(Paths.image('clouds'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);

				var bg:FlxSprite = new FlxSprite(-300, 0).loadGraphic(Paths.image('middle-mountain'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);

				var bg:FlxSprite = new FlxSprite(-1200, -450).loadGraphic(Paths.image('left-mountain'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);
//magnus if you are reading this dm Cherry#0821 and ask for
				var bg:FlxSprite = new FlxSprite(1300, 100);
				bg.frames = Paths.getSparrowAtlas('waterfall');
				bg.animation.addByPrefix('waterfall', 'waterfall', 24);
				bg.animation.play('waterfall');
				bg.scrollFactor.set(0.5, 0.5);
				bg.scale.set(2, 2);
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-700, 600).loadGraphic(Paths.image('groundbanjo'));
				...
				var stageFront:FlxSprite = new FlxSprite(1600, 400).loadGraphic(Paths.image('bridge'));
				stageFront.setGraphicSize(Std.int(stageFront.width * 0.75));
				stageFront.updateHitbox();
				stageFront.antialiasing = true;
				stageFront.scrollFactor.set(1, 1);
				stageFront.active = false;
		...
		gf = new Character(400, 0, gfVersion);
		//gf.scrollFactor.set(0.95, 0.95);

		dad = new Character(-300, 50, SONG.player2);
		...
		boyfriend = new Boyfriend(1170, 375, SONG.player1);
	...
	override function stepHit()
	{
		...
		if (curStep == 1242 && curSong == 'New-Moves')
			{
				boyfriend.playAnim('hey', false);
				dad.playAnim('guhhuh', false);
			}	
	}
